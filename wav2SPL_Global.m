function SPL_Global=wav2SPL_Global(ruta)

%   Cálculo del SPL de una grabación cada 10 segundos.
%
...
%%%%%%FIN PAULA.
    ...
    %%Filtramos
     ...
     SPLt_un=sum(Pi); %Almaceno la suma de esos 19 valores para el cacho de 10 segundos leído, en un.nat.
     SPLt_dB=10*log10(SPLt_un); %Lo paso a dBs.
     %SPLfrag_dB(i)siendo i el instante temporal. es decir, el número de cortes de
     %i que haya, de 10 segundos
...
%%%%%INICIO PAULA. 
     ...
     y=y+10*Fs; %hasta los 10 segundos posteriores
     %Si x es igual a la duración total en muestras, ó no completamos un
     %fragmento de 10 segundos:
...
%PAULA.Como G=31.60, me cargo el switch: Case 31.60 F=165; 
 ...
 SPL_Global=SPLfrag_dB+F; %PAULA. SPL=Ps+F; Tiene que haber tantas posiciones como fragmentos de 10 segundos a leer haya
 %%%%%%FIN PAULA.

 
 
%Paso los niveles de todos los tercios de octava a potencia Pi=10^(SPLi/10) ,SPLi = PR.
%Los sumo (Pi_Suma) y luego vuelvo a pasar a dB (Ptotal=10*log10(Pi_suma) )
%Calcular niveles cada 10 segundos. 
% https://es.mathworks.com/help/matlab/import_export/read-and-get-information-about-audio-files.html
